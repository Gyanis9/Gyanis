...
#define LOG_LEVEL(logger, level) \
    if((logger)->getLevel() <= (level)) \
        Gyanis::base::LogEventWrap(Gyanis::base::LogEvent::Create((logger), (level), std::source_location::current())).getSS()
/**
 * @brief 输出 debug 级别日志
 */
 ...
 */
#define LOG_FMT_LEVEL(logger, level, ...) \
    if(logger->getLevel() <= level) \
        Gyanis::base::LogEventWrap(Gyanis::base::LogEvent::Create((logger), (level), std::source_location::current())).getEvent()->format(__VA_ARGS__)

/**
 * @brief 输出 debug 级别格式化日志
 */
#define LOG_FMT_DEBUG(logger, ...) LOG_FMT_LEVEL(logger, Gyanis::base::LogLevel::DEBUG, __VA_ARGS__)

/**
 * @brief 输出 info 级别格式化日志
 */
#define LOG_FMT_INFO(logger, ...)  LOG_FMT_LEVEL(logger, Gyanis::base::LogLevel::INFO, __VA_ARGS__)

/**
 * @brief 输出 warn 级别格式化日志
 */
#define LOG_FMT_WARN(logger, ...)  LOG_FMT_LEVEL(logger, Gyanis::base::LogLevel::WARN, __VA_ARGS__)

/**
 * @brief 输出 error 级别格式化日志
 */
#define LOG_FMT_ERROR(logger, ...) LOG_FMT_LEVEL(logger, Gyanis::base::LogLevel::ERROR, __VA_ARGS__)

/**
 * @brief 输出 fatal 级别格式化日志
 */
#define LOG_FMT_FATAL(logger, ...) LOG_FMT_LEVEL(logger, Gyanis::base::LogLevel::FATAL, __VA_ARGS__)
...
namespace Gyanis::base
{
    ...
    public:
        ...
        /**
         * @brief 通过 C++20 source_location 创建日志事件
         */
        static std::shared_ptr<LogEvent> Create(const std::shared_ptr<Logger> &logger,
                                                LogLevel::Level                level,
