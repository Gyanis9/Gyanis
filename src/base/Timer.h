...
namespace Gyanis::base
{
    ...
    struct Timer
    {
        ...
        TimerClock::time_point    expired;   /// 定时器的过期时间点
         ...
         */
        explicit Timer(uint64_t id, std::chrono::milliseconds interval, std::function<void()> callback, bool recurring);
    ...
    public:
         ...
         * @return 返回定时器的唯一 ID
         */
        uint64_t addConditionTimer(uint64_t interval, std::function<void()> callback, std::weak_ptr<void> weak_cond, bool recurring = false);
         ...
         */
        [[nodiscard]] bool refresh(uint64_t id);

        /**
         * @brief 重置定时器的过期时间
         */
        [[nodiscard]] bool reset(uint64_t id, uint64_t interval, bool from_now);

        /**
         * @brief 获取下一个定时器的剩余时间
         */
        [[nodiscard]] std::chrono::milliseconds getNextTimer();
    ...
    private:
        ...
        TimerClock::time_point                                                         m_last_check_time; ///< 上次检查的时间，用于时间回滚检测
    };
}
