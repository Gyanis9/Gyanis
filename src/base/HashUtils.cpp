...
#define	ROTL(x, r) ((x << r) | (x >> (32 - r)))
    ...
    uint32_t murmur3_hash(const void* data, const uint32_t& size, const uint32_t& seed)
    {
        ...
        uint32_t s, h = seed;
        const auto ptr = reinterpret_cast<const uint32_t*>(str);
        constexpr uint32_t seed2 = 0x1b873593;
        constexpr uint32_t seed1 = 0xcc9e2d51;

        const auto len = size;
        const auto blk = len / 4;
        for (int i = 0; i < blk; i++)
        {
            s = ptr[i];
            s *= seed1;
        ...
        s = 0;
        auto* tail = reinterpret_cast<const uint8_t*>(str + blk * 4);
        switch (len & 3)
        {
        case 3: s |= tail[2] << 16;
        case 2: s |= tail[1] << 8;
    ...
    uint32_t murmur3_hash(const char* str, const uint32_t& seed)
    {
        ...
        // handle tail
        s = 0;
        const auto* tail = reinterpret_cast<const uint8_t*>(str + blk * 4);
        switch (len & 3)
        {
        case 3: s |= tail[2] << 16;
        case 2: s |= tail[1] << 8;
    ...
    {
        const auto str = static_cast<const char*>(tmp);
        unsigned int h = 0;
    ...
    {
        return (static_cast<uint64_t>(murmur3_hash(str, size, seed)) << 32 | murmur3_hash(str, size, seed2));
    }

    uint64_t murmur3_hash64(const char* str, const uint32_t& seed, const uint32_t& seed2)
    {
        return (static_cast<uint64_t>(murmur3_hash(str, seed)) << 32 | murmur3_hash(str, seed2));
    }
    ...
    std::string sha0sum(const void* data, const size_t len)
    {
        SHA_CTX ctx;
        SHA1_Init(&ctx);
        ...
        SHA1_Final(reinterpret_cast<unsigned char*>(&result[0]), &ctx);
        return result;
    }
    ...
    struct xorStruct
    {
        explicit xorStruct(const char value) : m_value(value)
        {
        }

        char m_value;
        char operator()(const char in) const { return in ^ m_value; }
    };
    ...
    std::string hmac(const std::string& text, const std::string& key)
    {
        ...
        if (keyLocal.size() > B)
        {
            ...
            keyLocal.resize(L);
            Final(reinterpret_cast<unsigned char*>(&keyLocal[0]), &ctx);
        }
        ...
        std::string result;
        result.resize(L);
        Final(reinterpret_cast<unsigned char*>(&result[0]), &ctx);
        Init(&ctx);
    ...
    void
    hexstring_from_data(const void* data, const size_t len, char* output)
    {
    ...
    std::string
    hexstring_from_data(const void* data, const size_t len)
    {
        if (len == 0)
        {
            return {};
