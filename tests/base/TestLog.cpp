...
{
    const auto logger = LOG_NAME("stress.macro");
...
TEST_CASE("file appender supports plain file name", "[base][log][stress]")
{
    ...
    const auto appender = std::make_shared<Gyanis::base::FileLogAppender>(file.string());
    const auto logger   = LOG_NAME("stress.file");
...
TEST_CASE("log event format keeps long payload", "[base][log][stress]")
{
    ...
    const auto event  = std::make_shared<Gyanis::base::LogEvent>(
        logger,
...
TEST_CASE("logger stream benchmark", "[base][log][stress][benchmark]")
{
    ...
    const auto logger = LOG_NAME("stress.benchmark");
    logger->setLevel(Gyanis::base::LogLevel::DEBUG);
    logger->clearAppenders();
